...
public class Backup_PromotedPMIEvaluation {
	...
	public Backup_PromotedPMIEvaluation(String InfileName, String pos) {
		...
		try{
			...
			statPrecisionAllOut = new PrintWriter(new FileWriter(infileName.substring(0, infileName.length() - 4) + "_promoted_pr_all.txt", true));
			statPrecision1SenseOut = new PrintWriter(new FileWriter(infileName.substring(0, infileName.length() - 4) + "_promoted_pr_1sense.txt", true));
			statPrecision1SynonymOut = new PrintWriter(new FileWriter(infileName.substring(0, infileName.length() - 4) + "_promoted_pr_1synonym.txt", true));
			
			statRecallAllOut = new PrintWriter(new FileWriter(infileName.substring(0, infileName.length() - 4) + "_promoted_re_all.txt", true));
			statRecall1SenseOut = new PrintWriter(new FileWriter(infileName.substring(0, infileName.length() - 4) + "_promoted_re_1sense.txt", true));
			statRecall1SynonymOut = new PrintWriter(new FileWriter(infileName.substring(0, infileName.length() - 4) + "_promoted_re_1synonym.txt", true));
	...
	public static void main(String[] args) throws Exception {
		...
		Backup_PromotedPMIEvaluation test;
		int totalEntries;
		
		if (args.length > 0){
			
			if (args[0].equals("VB")){
				test = new Backup_PromotedPMIEvaluation(dataPath+"/evaluation/verbSynonyms700.txt", "VB");
				totalEntries = 1187;
				
			}else if (args[0].equals("NN")){
				test = new Backup_PromotedPMIEvaluation(dataPath+"/evaluation/nounSynonyms700.txt", "NN");
				totalEntries = 2286;
				
			}else if (args[0].equals("JJ")){
				test = new Backup_PromotedPMIEvaluation(dataPath+"/evaluation/adjSynonyms700.txt", "JJ");
				totalEntries = 1015;
				
			}else if (args[0].equals("RB")){
				test = new Backup_PromotedPMIEvaluation(dataPath+"/evaluation/advSynonyms700.txt", "RB");
				totalEntries = 39;
			}else{
		...
		*/
		...
		test.getStatisticForAllSenses(k, p, q, bsf, 0);
		test.reset();
